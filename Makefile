...
.PHONY: build
build: web-build manifests generate fmt vet ## Build manager binary.
...
.PHONY: web-build
web-build: ## Build the embedded React/TypeScript web interface.
	cd web && "$(NPM)" ci && "$(NPM)" run build && rm -rf node_modules
